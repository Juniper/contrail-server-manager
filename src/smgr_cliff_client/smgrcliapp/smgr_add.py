...
class Add(Command):
    ...
    def get_parser(self, prog_name):
        ...
        self.parameter_classes["server"] = ["network", "parameters", "tag", "contrail"]
        self.parameter_classes["cluster"] = ["parameters"]
        ...
        for param in object_dict["server"]:
            if param not in self.parameter_classes["server"]:
                parser_server.add_argument(
                    "--" + str(param),
                    help="Parameter " + str(param) + " for the server being added",
                    default=None
                )
        for param in self.parameter_classes["server"]:
            parser_server.add_argument(
                "--" + str(param),
                help="Configure " + str(param) + " details for the server being added",
                nargs='*',
                default=None
            )
    ...
    def get_default_object(self, obj):
        ...
        default_object = {}
        config_object_defaults = self.get_object_config_ini_entries(obj)
    ...
    def take_action(self, parsed_args):
        ...
        try:
            if getattr(parsed_args, "file_name", None) and obj:
                payload = json.load(open(parsed_args.file_name))
                #self.merge_with_defaults(obj, payload)
            elif obj:
                payload = dict()
                payload[obj] = []
                resp = self.app.send_REST_request(
                ...
                tag_dict = json.loads(resp)
                rev_tag_dict = dict((v, k) for k, v in tag_dict.iteritems())
                if obj not in self.mandatory_params.keys():
                    payload = None
                else:
                    mandatory_params_list = self.mandatory_params[obj]
                    object_payload = {}
                    for param in object_dict[obj]:
                        param_classes = self.parameter_classes[obj]
                        if param not in param_classes and not str(param).startswith("tag"):
                            param_value = getattr(parsed_args, param, None)
                            if param_value:
                                object_payload[param] = param_value
                            """elif param in mandatory_params_list:
                                self.app.stdout.write("\nMandatory parameter " + str(param) + " for object " +
                                                      str(obj) + " doesn't have any value\n")
                                sys.exit(0)"""
                        elif obj == "tag" and str(param).startswith("tag") and param in rev_tag_dict:
                            param_value = getattr(parsed_args, param, None)
                            if param_value:
                                object_payload["tag"][param] = param_value
                        elif param in param_classes and getattr(parsed_args, param, None):
                            param_args = getattr(parsed_args, param, None)
                            if len(param_args) > 2:
                                raise ValueError('Wrong format when adding config for ' + str(param))
                            param_type = param_args[0]
                            if str(param_type).startswith("type="):
                                param_type = str(param_type).split('=', 2)[1]
                                if param_type == "dict":
                                    key_value_list = str(param_args[1]).split(',')
                                    key_value_dict = dict()
                                    for key_value_pair in key_value_list:
                                        key = str(key_value_pair).split("=", 2)[0]
                                        val = str(key_value_pair).split("=", 2)[1]
                                        key_value_dict[key] = val
                                    object_payload[param] = key_value_dict
                                elif param_type == "json":
                                    json_file = str(param_args[1])
                                    object_payload[param] = json.load(open(json_file))
                                else:
                                    raise ValueError('Wrong input type for added config ' + str(param) +
                                                     "\nAllowed = dict,json")
                            else:
                                raise ValueError('Wrong format when adding config for ' + str(param))
                    payload[obj].append(object_payload)
                    #self.merge_with_defaults(obj, payload)
            else:
                self.app.stdout.write("\nNo Object specified in Add command\n")
                sys.exit(0)
        ...
        if payload:
            resp = self.app.send_REST_request(self.smgr_ip, self.smgr_port, obj=obj, payload=payload, method="PUT")
            smgr_client_def.print_rest_response(resp)
            self.app.stdout.write("\n Payload = " + str(payload) + "\n")
            self.app.stdout.write("\n" + str(smgr_client_def.print_rest_response(resp)) + "\n")
        else:
            self.app.stdout.write("\nMandatory parameters for object " + str(obj) + " not entered\n")
