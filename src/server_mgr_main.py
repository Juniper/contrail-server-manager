...
#            match_value = match_values[0]
    ...
    def reimage_server(self):
        ...
        try:
            ...
            for server in servers:
                ...
                _mandatory_reimage_params = ["server_password", 
                            "server_gateway","server_domain", "ipmi_address", "server_ifname"]

                msg = ''
                for field in _mandatory_reimage_params:
                    if field not in reimage_parameters or \
                        reimage_parameters[field] == '' or \
                        reimage_parameters[field] == None:
                        msg += "%s " % field
                if msg != '':
    ...
    def get_defaults(self):
        ...
        try:
            ...
            if not level:
                ...
                self.merge_dict(ret_defaults_dict, self._code_defaults)
                return ret_defaults_dict[obj]
            elif level == "code":
                return self._code_defaults[obj]
            elif level == "config":
    ...
    def _read_smgr_object_defaults(self, obj_file):
       ...
       # server_params_defaults_dict = dict(smgr_obj_cfg_defaults.items("SERVER_PARAMS"))
        ...
        image_defaults_dict = dict(smgr_obj_cfg_defaults.items("IMAGE"))
