...
class VncServerManager():
    ...
    def get_vns(self):
        ...
        self._smgr_trans_log.log(bottle.request,
                                 self._smgr_trans_log.GET_SMGR_CFG_VNS)
        for x in entity:
            if x.get("vns_params", None) is not None:
                x['vns_params'] = eval(x['vns_params'])
        return {"vns": entity}
    # end get_vns
...
#                data.pop(data_item_key, None)
    ...
    def validate_smgr_reboot(self, validation_data, request , data=None):
        ...
        if len(entity) == 0:
            ...
            match_key, match_value = entity.popitem()
            # check that match key is a valid one
            if (match_key not in ("server_id", "mac",
                                  "tag", "vns_id")):
                msg = "Invalid Query arguments"
                raise ServerMgrException(msg)
        else:
    ...
    def send_status_mail(self, server_id, event, message):
        ...
        servers = self._serverDb.get_server(
            {"server_id" : server_id}, detail=True)
        if not servers:
    ...
    def upload_image(self):
        ...
        db_images = self._serverDb.get_image(
            {'image_id' : image_id}, detail=False)
        if db_images:
            abort(
                404,
    ...
    def delete_vns(self):
        ...
        try:
            ...
            if ret_data["status"] == 0:
                ...
                if match_key:
                    match_dict[match_key] = match_value
                self._serverDb.delete_vns(match_dict)
        except ServerMgrException as e:
            self._smgr_trans_log.log(bottle.request,
    ...
    def reimage_server(self):
        ...
        try:
            ...
            images = self._serverDb.get_image(
                {"image_id" : base_image_id}, detail=True)
            packages = self._serverDb.get_image(
                {"image_id" : package_image_id}, detail=True)
            if len(images) == 0:
            ...
            for server in servers:
                ...
                if 'esx_server' in server_params.keys():
                    ...
                    esx_servers = self._serverDb.get_server(
                        {'server_id' : server_params['esx_server']},
                        detail=True)
    ...
    def _do_provision_server(self, provision_params):
        try:
            ...
            server_id = provision_params['server_id'] + "." + \
                provision_params.get('domain', '')
            rc = subprocess.call(
                ["puppet", "kick", "--host", server_id])
