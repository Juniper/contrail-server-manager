...
--device %s --members %s --bond-opts \"%s\" --ip %s --no-restart-network\n") % \
    ...
    def find_package_sku(self, image_id):
        ...
        if version != '' :
            ...
            # following mapping is taken from 
            # https://wiki.openstack.org/wiki/Release_Naming
            if ver2.startswith('2014.1'):
              rel_name = 'icehouse'
            elif ver2.startswith('2014.2'):
              rel_name = 'juno'
            elif ver2.startswith('2015.1'):
              rel_name = 'kilo'
            elif ver2.startswith('2015.2'):
              rel_name = 'liberty'
            elif ver2.startswith('2016.1'):
              rel_name = 'mitaka'
            elif ver2.startswith('2016.2'):
              rel_name = 'newton'
            elif ver2.startswith('2017.1'):
              rel_name = 'ocata'

            self._smgr_log.log(self._smgr_log.DEBUG, "release of nova-api : %s" %rel_name)
            return rel_name
        else:
