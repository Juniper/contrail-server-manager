...
class Server(object):
    ...
    def verify_self_host(self):
        ...
        host_cmd = 'grep %s.%s /etc/hosts | grep -v "^[ ]*#"' %(self.id, self.domain)
        ...
        if status:
            ...
            server_cmd = 'sed -i "/agent/ a  \   \ server = %s.%s" /etc/puppet/puppet.conf' %(self.id, self.domain)
            self.exec_cmd(server_cmd, error_on_fail=True)
        #else:
