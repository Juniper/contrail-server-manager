...
mkdir -p $PWD/provision_logs/
log_file=$PWD/provision_logs/provision_$datetime_string.log
exec &> >(tee -a "$log_file")
...
if [ "$DEFAULT_DOMAIN" != "" ] && [ -f $SCRIPT_PATH/sm-client-config.ini ]; then
   sed -i "s|domain =.*|domain = ${DEFAULT_DOMAIN}|g" $SCRIPT_PATH/sm-client-config.ini
fi
