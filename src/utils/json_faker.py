...
def validate_JSON(data,combined_schema,schemafiles):
    ...
    if os.path.isfile(references_file):
        print "References file exists"
    else:
        print "References file does not exist"
...
def main():
    ...
    OptionParser.add_option('-s', '--input',action="append", dest="inputfiles",help="inputfile string", default=[])
    OptionParser.add_option('-o', '--output',action="store", dest="output",help="outputfile string", default=[])
    OptionParser.add_option('-v', '--validet',action="store", dest="validateIt",help="JSON file to be validated", default=[])
    ...
    if options.output:
        ...
        s = open(options.output,'r').read()
        for key in key_translations:
            s = s.replace(key, key_translations[key])
        ojson = json.loads(s)
        with open(options.output, 'w') as outfile:
            json.dump(ojson['contrail_4'], outfile, indent = 2)

    if options.validateIt:

        data = open(options.validateIt).read()
        data_dict = json.loads(data)
        merged_dict =(json.loads(merged_schema))
        validate_JSON(data_dict,merged_dict,options.inputfiles)
