...
def set_network(server, tap_name, br = 'br1', uplink = None):
    ...
    if uplink and uplink != 'None':
        e, o = run_command(server, "brctl addif %s %s" %(br, uplink))
        if e != 0:
            return 1, o
    return 0, []
...
def main():
    ...
    qemu_cmd = qemu_cmd + " -net nic,macaddr=%s,vlan=1" %args.mac
    qemu_cmd = qemu_cmd + " -net tap,ifname=%s,script=no,vlan=1" %(tap_intf)
    ...
    e, o = set_network(ssh_fd, tap_intf, args.bridge, args.uplink)
    if e:
        # delete VM
        delete_vm(ssh_fd, args.vmname)
        raise Exception("Error creating VM:\n%s\n" %o)
