...
check_image=`server-manager display image --image_id ${CONTRAIL_IMAGE_ID} --json | grep ${CONTRAIL_IMAGE_ID} | awk '{print $2}'`
while [[ "$check_image" != "\"${CONTRAIL_IMAGE_ID}\"" ]]
do
  sleep 15
  echo "Waiting for containers to get loaded"
  check_image=`server-manager display image --image_id ${CONTRAIL_IMAGE_ID} --json | grep ${CONTRAIL_IMAGE_ID} | awk '{print $2}'`
done
