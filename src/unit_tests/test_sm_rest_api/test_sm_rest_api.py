...
#Class for testing Server manager rest API calls for cluster object
class TestSMClusterRestApi(unittest.TestCase):
    #setup function which sets up environemnt required for unit testing
    @classmethod
    def setUpClass(cls):
        # Remove existing database and log files.
...
# end class TestSMClusterRestApi

#Create a TestSuite for the testing server manager rest api functionality
def sm_cluster_rest_api_suite():
    suite = unittest.TestSuite()
    suite.addTest(TestSMClusterRestApi('testGetClusterEmptyRestApi'))
    suite.addTest(TestSMClusterRestApi('testAddClusterRestApi'))
    suite.addTest(TestSMClusterRestApi('testGetClusterAllRestApi'))
    suite.addTest(TestSMClusterRestApi('testGetSpecificClusterRestApi'))
    suite.addTest(TestSMClusterRestApi('testGetNonexistingClusterRestApi'))
    suite.addTest(TestSMClusterRestApi('testModifyClusterRestApi'))
    suite.addTest(TestSMClusterRestApi('testDeleteClusterRestApi'))
    return suite


#Class for testing Server manager rest API calls for tag object
class TestSMTagRestApi(unittest.TestCase):
    #setup function which sets up environemnt required for unit testing
    @classmethod
    def setUpClass(cls):
        # Remove existing database and log files.
    ...
    def testModifyTagRestApi(self):
        ...
        newdata = newdata.replace('hall', 'hall_value')
        response = requests.put('http://%s:%s/tag' %(
            self.server_ip, self.server_port),data=newdata,headers = {'content-type': 'application/json'})
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(eval(response.content)['tag3'], 'hall_value')
...
# end class TestSMTagRestApi

#Create a TestSuite for the testing server manager rest api functionality
def sm_tag_rest_api_suite():
    suite = unittest.TestSuite()
    suite.addTest(TestSMTagRestApi('testGetTagEmptyRestApi'))
    suite.addTest(TestSMTagRestApi('testAddTagRestApi'))
    suite.addTest(TestSMTagRestApi('testGetTagAllRestApi'))
    suite.addTest(TestSMTagRestApi('testModifyTagRestApi'))
    suite.addTest(TestSMTagRestApi('testDeleteTagRestApi'))
    return suite
